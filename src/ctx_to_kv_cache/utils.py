...
def extract_cli_args(argv: list[str]):
    """Log-helper: dump --key=value CLI args into a YAML. Best-effort —
    space-separated list args (e.g. `--train_ds_names a b c` from nargs='+')
    are simply skipped instead of crashing.
    """
    out = dict()
    for elem in argv:
        if elem.endswith(".yaml"):
            out["config"] = elem
        elif elem.startswith("--") and "=" in elem:
            k, v = elem.split("=", 1)
            k = k[2:]
            v = try_convert(v)
            out[k] = v
