...
class Idefics2PerceiverResampler(Idefics2PreTrainedModel):
    ...
    def forward(
        ...
        # Opt-in gradient checkpointing on perceiver blocks: env
        # `HYPERX_PERCEIVER_GC=1` recomputes each block's forward during
        # backward → trades ~30% wall-time in the perceiver for ~3-4 GB of
        # autograd-saved activations (dominated by context RMSNorm fp32
        # buffers, ~600 MB/block × 9 blocks). Stacks with chunk-MB:
        # HYPERX_CHUNK_MICROBATCH=2 splits the perceiver into sub-streams,
        # and GC frees each sub-stream's activations after it produces its
        # output → combo unlocks ~5-6 GB headroom for n_lat bump.
        use_gc = self.training and os.environ.get("HYPERX_PERCEIVER_GC") == "1"
        for i, layer in enumerate(self.layers):
            ...
            if use_gc:
                layer_outputs = _checkpoint(
                    layer, use_reentrant=False, **attn_kwargs
                )
            else:
