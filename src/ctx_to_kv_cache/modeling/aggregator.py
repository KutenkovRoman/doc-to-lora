...
from ctx_to_kv_cache.modeling.idefics2 import (
    Idefics2Perceiver,
    Idefics2PerceiverConfig,
)
from ctx_to_kv_cache.utils import get_num_layers
...
class Perceiver(nn.Module):
    ...
    def forward(
        ...
        if self.layer_to_layer and not self.iterative_mode:
            # These two branches each consume the `num_layers` axis via a
            # rearrange; they are mutually exclusive by caller convention
            # (SFT/eval passes ctx_attn_mask, packed/training passes
            # ctx_position_ids). Enforce it in code: if BOTH were non-None
            # the first rearrange would drop num_layers and the second would
            # mis-reshape/crash silently. `elif` makes current callers
            # byte-identical and prevents the future footgun.
            assert not (
                ctx_attn_mask is not None and ctx_position_ids is not None
            ), (
                "layer_to_layer aggregator received BOTH ctx_attn_mask and "
                "ctx_position_ids; these branches are mutually exclusive "
                "(see comment)."
            )
